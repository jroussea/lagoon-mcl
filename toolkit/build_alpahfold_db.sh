...
apptainer run $LAGOON python $SCRIPT/download_alphafolddb.py --alphafold 1-AFDBClusters-entryId_repId_taxId.tsv

echo -e "\nEXTRACT ALPHAFOLD CLUSTER SEQUENCES\n"
apptainer run $SEQKIT seqkit grep -f sequences_id_alphafold_cluster_foldseek.txt sequences.fasta -o sequences_alphafold_clusters.fasta
...
apptainer run $MMSEQS2 mmseqs createdb ../sequences_alphafold_clusters.fasta alphafoldDB
...
apptainer run $LAGOON python $SCRIPT/uniprot.py --clusters 1-AFDBClusters-entryId_repId_taxId.tsv --uniprot protein2ipr.dat.gz
