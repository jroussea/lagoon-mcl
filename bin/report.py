...
def main(args):
    ...
    with open(f'report_{args.basename}.html', 'w') as f:
...
def read_edges_files(edges, d_clusters_metrics):
    ...
    with open(edges, "r") as f_edges:
        for line in f_edges:
            ...
            if l_line[14] not in d_edges.keys():
                d_edges[l_line[14]] = 1
            else:
                d_edges[l_line[14]] += 1
...
def read_nodes_metrics(nodes):
    ...
    with open(nodes, "r") as f_sequence:
        for position, line in enumerate(f_sequence):
            l_line = line.strip().split("\t")
            l_labels = l_line[4:-2]
            if position == 0:
                ...
                if l_line[1] not in d_metrics.keys():
                    d_metrics[l_line[1]] = {
                        'length': [int(l_line[2])],
                        'centrality': [float(l_line[3])]
                        }
                else:
                    d_metrics[l_line[1]]['length'].append(int(l_line[2]))
                    d_metrics[l_line[1]]['centrality'].append(float(l_line[3]))
                d_sequences["length"].append(int(l_line[2]))
                d_sequences["centrality"].append(float(l_line[3]))
                for pos, label in enumerate(l_labels):
...
def read_clusters_metrics(clusters, d_metrics):
    ...
    with open(clusters, "r") as f_cluster:
        for position, line in enumerate(f_cluster):
            l_line = line.strip().split("\t")
            l_hom_sc = l_line[3:]
            ...
            if position == 0:
                ...
                d_clusters_metrics["sequence_id"].append(l_line[0])
                d_clusters_metrics["cluster_size"].append(int(l_line[1]))
                d_clusters_metrics["diameter"].append(float(l_line[2]))
                d_clusters_metrics["mean_centrality"].append(float(np.round(np.mean(d_metrics[l_line[0]]["centrality"]), 3)))
                d_clusters_metrics["mean_length"].append(float(np.round(np.mean(d_metrics[l_line[0]]["length"]), 3)))
                d_clusters_metrics["max_centrality"].append(float(np.round(np.max(d_metrics[l_line[0]]["centrality"]), 3)))
                d_clusters_metrics["max_length"].append(int(np.round(np.max(d_metrics[l_line[0]]["length"]), 3)))
                
                for pos, l_label in enumerate(ll_hom_sc):
                    label = d_position[pos]
                    if l_label[0] != "NA":
                        unannotated_sequence = int(l_line[1])-int(l_label[1])
                        
                        d_homogeneity[label]["cluster_id"].append(l_line[0])
                        ...
                        d_homogeneity[label]["prop_annot"].append(float(round(int(l_label[1])/int(l_line[1])*100, 3)))
                        d_homogeneity[label]["cluster_size"].append(int(l_line[1]))
                        d_homogeneity[label]["seq_unannot"].append(unannotated_sequence)
                        d_homogeneity[label]["prop_unannot"].append(float(round(int(unannotated_sequence/int(l_line[1]))*100, 3)))
    
                        d_clst_caracteristics[label]["clst_annot"].append(int(l_line[1]))
                        
                    elif l_label[0] == "NA":
                        d_clst_caracteristics[label]["clst_unannot"].append(int(l_line[1]))
...
def reports_sequences_metrics(d_sequences, basename):
    ...
    plt.savefig(f"figures_{basename}/sequence_length_centrality.png", format="png")
    ...
    data_sequence_metrics = {
        'plot': f"figures_{basename}/sequence_length_centrality.png",
...
def reports_sequences_label(d_counter_label, basename):
    ...
    for label, d_counter in d_counter_label.items():
        ...
        for p in ax.patches:
            ax.annotate(
                f'{p.get_height()}%',  # Texte à afficher (la valeur de chaque barre)
                (p.get_x() + p.get_width() / 2., p.get_height()),  # Position du texte (au sommet de chaque barre)
                ha='center',  # Alignement horizontal (au centre de la barre)
                va='center',  # Alignement vertical (au sommet de la barre)
                fontsize=12,  # Taille de la police
                color='black',  # Couleur du texte
                xytext=(0, 5),  # Décalage du texte pour le mettre un peu au-dessus
                textcoords='offset points'  # Utilisation de l'unité de points pour le décalage
            )
            
        plt.tight_layout()

        plt.savefig(f"figures_{basename}/sequence_label_{label}.png", format="png")
        ...
        data_sequence_label[name] = {
            'plot': f"figures_{basename}/sequence_label_{label}.png",
...
def reports_clusters_metrics(d_clusters_metrics, basename):
    ...
    plt.savefig(f"figures_{basename}/clusters_metrics.png", format="png")
    ...
    data_clusters_sequences = {
        'plot': f"figures_{basename}/clusters_metrics.png",
...
def reports_clusters_labels(d_homogeneity, d_clst_caracteristics, basename):
    ...
    for label, d_metrics in d_homogeneity.items():
        ...
        plt.savefig(f"figures_{basename}/clusters_caracteristics_{label}.png", format="png")
        ...
        plt.savefig(f"figures_{basename}/homogeneity_score_{label}.png", format="png")
        ...
        data_clusters_labels[label] = {
            "plot1": f"figures_{basename}/clusters_caracteristics_{label}.png",
            "dataframe1": df_caracteristics_html,
            "plot2": f"figures_{basename}/homogeneity_score_{label}.png",
